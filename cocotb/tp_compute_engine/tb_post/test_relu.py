"""
cocotb testbench for relu_int16 (synchronous, clk/rst_n/start/busy/valid)
...
async def test_relu_reset_midway(dut):
    """Assert rst_n (active-low) while the FSM is busy and confirm it returns cleanly to IDLE"""
    cocotb.start_soon(Clock(dut.clk, CLK_PERIOD_NS, unit="ns").start())
    await reset_dut(dut)
